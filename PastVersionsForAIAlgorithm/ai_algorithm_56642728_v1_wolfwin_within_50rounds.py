...
class Board:
    ...
    def evaluate(self, player, gameEnds):
        if gameEnds:
            if self.winner == player:
                ...
                return -1000
        if player == 2:
            return (10 - self.check_num_of_sheep()) * 100 - self.check_total_distance_from_sheep_to_wolf()
        elif player == 1:
    ...
    def game_ends(self):
        ...
        if sheep_cnt <= 2:
            ...
            return True
        if (wolf1_exist and wolf1_trapped and not wolf2_exist) or (
                wolf1_exist and wolf2_exist and wolf1_trapped and wolf2_trapped):
...
def getBestMove(board, maxDepth, player):
    def ab_negamax(board, player, maxDepth, currentDepth, alpha, beta):
        ...
        if gameEnds or currentDepth == maxDepth:
            return None, None, None, None, board.evaluate(player, gameEnds)
        ...
        for move in all_moves:
            ...
            _, _, _, _, currentScore_ = ab_negamax(newBoard, player, maxDepth,
                                                                              currentDepth + 1, -beta,
                                                                              -max(alpha, bestScore))
    ...
    best_start_row, best_start_col, best_end_row, best_end_col, bestScore = ab_negamax(board, player, maxDepth, 0,
                                                                                       -math.inf, math.inf)
    return best_start_row, best_start_col, best_end_row, best_end_col, bestScore
...
def AIAlgorithm(filename, movemade):  # a showcase for random walk
    ...
    if movemade == True:
        ...
        [start_row, start_col, end_row, end_col, scores] = getBestMove(board, 5, 2)
        print("Wolf Scores: ", scores)
    ...
    if movemade == False:
        ...
        [start_row, start_col, end_row, end_col, scores] = getBestMove(board, 5, 1)
        print("Sheep Scores: ", scores)
        matrix2 = copy.deepcopy(matrix)
